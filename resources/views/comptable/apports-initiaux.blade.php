...
<div class="container-fluid">
    <div class="row mb-4">
        <div class="col-12">
            <h2 class="page-title">Apports Initiaux</h2>
        </div>
    </div>
    ...
    <div class="row mb-4">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form method="GET" action="{{ route('comptable.apports-initiaux') }}" class="row g-3">
                        <div class="col-md-4">
                            <label for="search" class="form-label">Recherche</label>
                            <input type="text" class="form-control" id="search" name="search" value="{{ request('search') }}" placeholder="Client, projet...">
                        </div>
                        <div class="col-md-3">
                        ...
                        <div class="col-12">
                            ...
                            <a href="{{ route('comptable.apports-initiaux') }}" class="btn btn-secondary"><i class="fas fa-undo"></i> Réinitialiser</a>
                        </div>
                    </form>
                </div>
            </div>
                ...
                <div class="table-responsive">
                    <table class="table-custom">
                        ...
                        <tbody>
                            ...
                            @empty
                            <tr>
                                <td colspan="8" class="text-center">Aucun apport initial trouvé</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
                </div>
                <div class="mt-3">{{ $apportsInitiaux->links() }}</div>
            </div>
        </div>
    </div>
</div>

<!-- Modals pour paiement -->
@foreach($apportsInitiaux as $paiement)
<div class="modal fade" id="payerModal{{ $paiement->id }}" tabindex="-1" aria-labelledby="payerModalLabel{{ $paiement->id }}" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="payerModalLabel{{ $paiement->id }}">Payer - Apport Initial</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('comptable.apports-initiaux.payer', $paiement) }}" method="POST" enctype="multipart/form-data">
                @csrf
